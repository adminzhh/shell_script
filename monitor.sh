...
mem_total=$(free -h | awk '/Mem/{print $2}')               #总内存容量
mem_free=$(free -h | awk '/Mem/{print $NF}')               #剩余内存容量
swap_total=$(free -h | awk '/Swap/{print $2}')             #总swap容量
swap_free=$(free -h | awk '/Swap/{print $NF}')             #剩余swap容量
disk=$(df -h | awk '/^\/dev/{print $1,$2,$4}'|column -t)   #磁盘信息
